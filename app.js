...
// Новая команда для выдачи любой суммы
...
bot.on('successful_payment', (ctx) => {
    ...
    if (users[userId]) { users[userId].balance += amount; saveDB(); ctx.reply("💎 Баланс пополнен!"); }
});
...
bot.start(async (ctx) => {
    ...
    if (!users[userId]) {
        users[userId] = { username: ctx.from.username || 'unknown', balance: 1000000, usedPromos: [], completedTasks: [], tradeLink: "", invitedBy: null };
...
app.listen(PORT, '0.0.0.0', () => { console.log(`Live on ${PORT}`); bot.launch(); });
